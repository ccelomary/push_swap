		...
		else
		{
			if (!is_sorted2(*current_st, args.comp))
				push_selectednumber(current_st, reverse_st, args.iterchunk,
					init_args(args.args.chunk,
						get_last_one, args.args.len_st, args.s));
			else
				push_selectednumber(current_st, reverse_st, args.iterchunk,
					init_args(args.args.chunk,
						get_first_one, args.args.len_st, args.s));
		}
	}
	push_selectednumber(current_st, reverse_st, args.iterchunk,
...
int (*get_numberpvt)(t_stack *st, int pivot,
int chunk, int (*compare)(int a, int b)))
{
	int		pivot;
	int		number;
	int		position;
